...
`NimbusTests` держит две проверки:

- `EndpointSmokeTests` — 47 чтений вживую с токеном из Keychain, включая курсоры `next_href` (ими работает вся подгрузка лент, и ломаются они отдельно от первых страниц). Таблица: `OK` / HTTP-код / имя пропавшего поля.
- `WritePathTests` — 16 путей записи **без токена**: api-v2 отвечает 401 на живой путь и 404 на неизвестный (тем же приёмом пути и устанавливались, см. `SCWrite.blockUser`). Ничего не меняет в аккаунте.

Чего проверки не ловят: проходит ли запись на самом деле — форма тела и DataDome-контекст доказываются только ручным лайком. `reportComment` не пробуется даже неавторизованно: жалобу нельзя отменить.

GraphQL-операции каталогом не покрыты: там меняется не путь, а текст запроса, и все семь лежат рядом в `Endpoints/Comments.swift`.
