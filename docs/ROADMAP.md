...
- ~~**Preload**~~ — **сделано 07.09.2026** (`6b74085`..`22f4f0d`). Транспорт переведён на две деки `AVPlayer`: следующий трек строится за 20 с до конца и буферизуется в резервной деке, переход — своп дек. Замер: 20–34 мс до звука против 1477 мс раньше. Опасение из этого пункта — «preload N+1 требует второй `AVContentKeySession`, крупно и рискованно» — проверено вживую: при прогреве две FairPlay-сессии сосуществуют ~20 с и воспроизведение не ломается. **Кроссфейд остаётся вырезанным**, но архитектура его держит: две деки — это два независимых выхода с раздельной громкостью, останется рампа.
...
- ~~**Настоящий Sign Out**~~ — сделано: `signOut` (`AppModel.swift:52`) чистит Keychain, гасит сессию плеера, сбрасывает библиотеку и роняет web-хранилище через `WebSessionCookies.clear()`. Полнота самого сброса библиотеки дозакрыта коммитом `25bc56e`.
...
~~6. **Deployment target**~~ — закрыт 16.09.2026: пол поднят до macOS 26.0 во всех четырёх конфигах, `ARCHS = arm64`, фолбэки на материалах удалены.